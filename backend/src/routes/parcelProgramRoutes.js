...
router.post('/', authenticateToken, requireRole('OWNER', 'ADMIN'), async (req, res) => {
  try { const { name, year, targetAmount, notes } = req.body || {}; if (!name || !Number(year) || Number(targetAmount) <= 0) return res.status(400).json({ success: false, message: 'Nama program, tahun, dan target wajib diisi.' }); const row = await prisma.parcelProgram.create({ data: { name: String(name).trim(), year: Number(year), targetAmount: Number(targetAmount), notes: notes ? String(notes).trim() : null }, include: { _count: { select: { participants: true } } } }); return res.status(201).json({ success: true, program: format(row) }); }
