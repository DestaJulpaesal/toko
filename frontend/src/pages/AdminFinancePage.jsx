...
export default function AdminFinancePage() {
  ...
  const load = async () => {
    setLoading(true);
    try {
      const [summaryResponse, transactionResponse, categoryResponse, accountResponse, cashflowResponse, breakdownResponse, tagResponse] = await Promise.all([
        apiFetch('/finance/summary'), apiFetch('/finance/transactions'), apiFetch(`/finance/categories?type=${form.type}`), apiFetch('/finance/accounts'), apiFetch('/finance/cashflow?period=daily&months=1'), apiFetch('/finance/breakdown?type=EXPENSE'), apiFetch('/finance/tags'),
      ]);
